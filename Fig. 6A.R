...
library(lattice)
library(ggrepel)
data <- read.delim(file = "ko.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE, header = T)
cmic_group <- read.delim(file = "group-ko.txt")
...
library(ggplot2)
color=c('#6EA2DB', '#45A8AC', '#DC8F95', '#3751A6', '#4daf4a', '#ED5F23')#颜色变量
p1 <- ggplot(cmic_pcoa_result, aes(x=PCoA1, y=PCoA2, color=ST, fill=ST, shape = ST)) + #shape=ST
  labs(x=paste("PCoA axis1 (", eig_percent[1], "%)", sep=""),
       y=paste("PCoA axis2 (", eig_percent[2], "%)", sep="")) +
  geom_point(size=1.5, alpha=0.75) +
  #stat_ellipse(aes(group=ST), # 椭圆仅用线型区分,, linetype=Phylum
  #             linetype = 2,
  #             geom = "polygon",
  #             level=0.95,alpha=0.3) +    # 设置椭圆线宽
  #scale_fill_manual(values = color)+#椭圆颜色
  scale_shape_manual(values = c(19,19,19,17,17,17)) +
  scale_color_manual(values = c('#6EA2DB', '#45A8AC', '#DC8F95', '#3751A6', '#4daf4a', '#ED5F23')) +
